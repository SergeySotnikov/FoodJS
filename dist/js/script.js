...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function bindPostData(form) {
    form.addEventListener("submit", e => {
      ...
      postData("http://localhost:3000/requests", json).then(request => {
        console.log(request);
  ...
  next.addEventListener("click", () => {
    if (offset === (+width.slice(0, width.length - 2) + margin) * (slides.length - 1)) {
      offset = 0;
    } else {
  ...
  previews.addEventListener("click", () => {
    if (offset === 0) {
      offset = (+width.slice(0, width.length - 2) + margin) * (slides.length - 1);
    } else {
      offset -= +width.slice(0, width.length - 2) + margin;
    }
